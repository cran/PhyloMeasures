...
#ifndef R_STYLE_SAMPLER_H
#define R_STYLE_SAMPLER_H
...
class R_style_sampler
{
 ...
 private:
  ...
  int _select_random_element_recursive(Number_type p, int index)
  {
    ...
    /*
    std::cout << "##################################################### " << std::endl;
    std::cout << " Select random lmnt recursive on node with index: " << index << std::endl; 
    std::cout << " p: " << p << std::endl; 
    std::cout << " Abundance: " << v.abundance << std::endl; 
    std::cout << " Leaves in subtree: " << v.leaves_in_subtree << std::endl; 

    if(v.lc == -1)
      std::cout << " Is child node " << std::endl;
    else
    {
      std::cout << " Left child: " << v.lc << std::endl;     
      std::cout << " Right child: " << v.rc << std::endl;     
      std::cout << " Abundance of left child: " << _tree[v.lc].abundance << std::endl; 
      std::cout << " Abundance of right child: " << _tree[v.rc].abundance << std::endl; 
      std::cout << " Leaves in subtree left child: " << _tree[v.lc].leaves_in_subtree << std::endl;
      std::cout << " Leaves in subtree right child: " << _tree[v.rc].leaves_in_subtree << std::endl;  
    }
    */
    ...
    // Unfortunately, the following check causes problems due to numeric imprecision 
    // (p might sometimes be infinitesimally larger than v.abundance).

    //if(v.abundance < p)
    //{
    //  std::string exception_msg;
    //  exception_msg += " Unexpected error during weighted sampling.\n";     
    //  Exception_type excp;
    //  excp.get_error_message(exception_msg);
    //  Exception_functor excf;
    //  excf(excp);
 ...
 public:
  ...
  R_style_sampler(Sampler_data data, unsigned seed):
  _generator(seed),_distribution(0.0,1.0)
  { _initialize(*(data.vals),*(data.abundances)); }

  R_style_sampler(Sampler_data data):
  _generator(std::chrono::system_clock::now().time_since_epoch().count()),_distribution(0.0,1.0)
  { _initialize(*(data.vals),*(data.abundances)); }

  R_style_sampler(std::vector<int> &vals, std::vector<Number_type> &abundances, unsigned seed):
  _generator(seed),_distribution(0.0,1.0)
  { _initialize(vals,abundances); }

  R_style_sampler(std::vector<int> &vals, std::vector<Number_type> &abundances):
  _generator(std::chrono::system_clock::now().time_since_epoch().count()),_distribution(0.0,1.0)
  { _initialize(vals,abundances); }
